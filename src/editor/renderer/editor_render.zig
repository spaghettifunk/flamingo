...
pub fn setVirtualCursor(editor: anytype, ctx: RenderContext) void {
    ...
    if (editor.state.mode == .FilesystemPicker or editor.state.mode == .Prompt or editor.state.mode == .Help or editor.state.mode == .GitGraph or editor.state.mode == .GitDiff or
